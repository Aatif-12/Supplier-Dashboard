...
from crud import create_compliance_record
from crud import (
    get_supplier_compliance_history,
)
from schemas import ComplianceCheck, ComplianceRecord, InsightsResponse
...
@router.post("/suppliers/check-compliance", response_model=ComplianceRecord)
def check_compliance(data: ComplianceCheck, db: Session = Depends(get_db)):
    # Create the compliance record in the database
    record = create_compliance_record(db, data)

    # Return the record using the Pydantic model
    return record
